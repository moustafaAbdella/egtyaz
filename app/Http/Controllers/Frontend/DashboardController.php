...
class DashboardController extends FrontendController
{
    ...
    public function getSetting( $type ){
        if( is_array($type) ){
            $returnOptions = [] ;
            $options = $this->options->whereIn('type' , $type )->select([ 'type' ,'option_value' , 'option_var'])->get() ;
...
// dd($questions);
    ...
    public function courses(Request $request, $category = 0){
    ...
    public function examsSubjects($id){
        ...
        // $skills = $this->taxService->getSkills($id);
...
// $sub_id = $id;
            ...
            // $questions  = Posts::where('status' , 1)->where('type' , 'learn')->where('taxonomy_id' , $skill_id)->paginate(20) ;
return $this->view('frontend.dashboard.exams.list', [
    'list' => $data,
    'sub_id' => $id
]);
        return $this->view('frontend.dashboard.exams.list' , ['list' => $data ] ) ; 
