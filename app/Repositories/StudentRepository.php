...
class StudentRepository extends BaseRepository
{
    ...
    public function makeExam($data)
    {
        ...
        foreach ($data['subjects'] as $skill) {
            if (isset($skill['skills']) && is_array($skill['skills'])) {
    ...
    public function getExam($id, $limit = 0)
    {
        ...
        $questions = $this->exams->getExamQuestionsObject($id,$exam, $skills, $subjects, $exam->questions_count)
            ->with('answers')->with('attachment')->orderBy('id', 'ASC')->orderBy('updated_at', 'ASC')->get();
...
//        }
    ...
    public function getExamsBySubjects($id , $student_id = null)
    {
        $exam = Exam::whereHas('Subjects', function ($query) use($id) {
            if ($id == 2){ 
                $query->whereIn('subject_id' , ['111']);
                // $query->whereIn('subject_id' , ['23', '24']);

            } else if ($id == 23){ 
                $query->whereIn('subject_id' , ['23']);
                // $query->whereIn('subject_id' , ['23', '24']);

            } else if ($id == 54){ 
                $query->whereIn('subject_id' , ['54']);

            } else if ($id == 24){ 
                $query->whereIn('subject_id' , ['24']);
                // $query->whereIn('subject_id' , ['23', '24']);
        ...
        //     $exam = $exam->where('student_id', $student_id);
        // }
        // return $exam->where('type','!=','challenge')->where('type','!=','free')->orderBy('id', 'ASC')->orderBy('updated_at', 'ASC')->paginate(10);
        return $exam->where('type','!=','challenge')
                    ->where('type','!=','free')
                    ->withCount([
                        'answers as students_count' => function($q) {
