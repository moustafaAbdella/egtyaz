...
class PaymentService extends BaseService{
    ...
    public function __construct(PaymentRepository $paymentRepository, UserService $userService, PackageService $packageService, Courses $courses, PointsService $pointsService,  BankMisrService $visaService , PaytabsService $paytabsService , CouponService $couponService)
    {
    ...
    public function getUserPkgs($user_id){
       ...
       $pkgs->each(function($pkg) use(&$payments){   
          ...
          array_push($payments, [
              ...
              "endTime" => Carbon::parse($pkg->created_at)->addDays((int)$pkg->Package->period)->format("Y-m-d"),
    ...
    public function pkgHasAccess($user_id, $action){
       ...
       if($payment && $payment->package){
            ...
            if( 
                ...
                &&
                $dateNow < Carbon::parse($payment->updated_at)->addDays((int)$payment->package->period)
            ){
