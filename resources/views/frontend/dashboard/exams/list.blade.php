  ...
  @section('Content')
    ...
    <a href="{{ route('start_skills', ['subject_id' => $sub_id , 'category_id'=>1]) }}" class="btn btn-primary">
        <i class="flaticon-book"></i> عرض كل اختبارات المادة رقم {{ $sub_id }}
    </a>
</div>
		  ...
		  @foreach ($list as $exam)
			...
			@endphp
   		    <div class="testStyle clearfix @if($has_results) active  @endif">
				@if( $has_results )
    			<a href="{{route("ExamResult", ["exam" => $exam->id])}}" class="title">
				...
				&nbsp;
				<a href="{{route("startExam", ["exam" => $exam->id])}}" class="btn btn-sm btn-warning">اعادة الأختبار</a>
				@else
				<a href="{{route("startExam", ["exam" => $exam->id])}}" class="title">
					@if( isset($exam->title[3]) )
						{{ $exam->title }}
					@else
						الاختبار التجريبى رقم {{ $max }}
				...
				<ul class="date clearfix">
					...
					<li><i class="flaticon-user"></i> حلّ الاختبار {{ $exam->students_count }} طالبًا</li>
				</ul>
    		</div>
			@php $max-- ; @endphp
          @endforeach
