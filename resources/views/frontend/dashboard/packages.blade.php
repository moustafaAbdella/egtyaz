...
@include('frontend.includes.breadCrumbDashboard')	  
	    ...
	    <div class="Subscriptions  minHeight600">
    		...
    		{{-- primary packages --}}
    <div class="primaryPlanWraper">

        @foreach ($availablePkgs['primary'] as $package)
            @php
                $roles = (array) json_decode($package->roles);
            @endphp


            <div class="secondaryPlan">
             	<div class="prePlan">الأكثر مبيعًا</div>
                <div class="planHead">
                    ...
                    <h2 class="title text-center price">
                        ...
                        @endphp
                        @if ($diffPrice == 1)
                            <s class="text-danger"> {{ $package->price }} </s> {{ $package->new_price }}
                            {{ trans('currency.' . env('currency_code', 'SAR')) }}
                        @else
                        ...
                        @foreach ($PackageRoles as $k => $v)
                            <li class="clearfix"> {{ $v }}
                                @if (isset($roles[$k]) && $roles[$k] == true)
                                    <img src="{{ asset('/frontend/images') }}/checkedPlan.png" />
                            </li>
                        @else
        ...
        <a href="{{ route('buy_package_details', $package->id) }}" class="btnPlan">اشترك الآن</a>
    </div>
    </div>
    @endforeach
    </div>
    ...
    {{-- other packages --}}
    <div class="secondaryPlanWraper">

        @foreach ($availablePkgs['secondary'] as $package)
            @php
                $roles = (array) json_decode($package->roles);
            @endphp
